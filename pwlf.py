...
class PieceWiseLinear(object):
  ...
  def __init__(self, x0, y0, x1, y1):
    ...
    self.x1 = x1
    self.y1 = y1
    self._xy = [(self.x0, self.y0), (self.x1, self.y1)]
  ...
  def __str__(self):
    str = "(%g,%g).." % (self.x0, self.y0)
    if len(self._xy) > 2:
      for i in range(1, len(self._xy)-1):
        str += "(%g,%g).." % (self._xy[i][0], self._xy[i][1])
    str += "(%g,%g)" % (self.x1, self.y1)
    return str
  ...
  def __call__(self, x):
    ...
    if len(self._xy) > 1:
      for i in range(1, len(self._xy)):
        if x <= self._xy[i][0]:
          d_x = self._xy[i][0] - self._xy[i-1][0]
          d_y = self._xy[i][1] - self._xy[i-1][1]
          n = i-1
          break
        continue
    else:
  ...
  def join(self, rhs):
    ...
    f1 = PieceWiseLinear(d1[0], self.y0, d2[1], rhs.y1)
    if len(self._xy) > 2:
      for i in range(1, len(self._xy)-1):
        f1._xy.insert(i ,self._xy[i])
    f1._xy.insert(-1, (d1[1], self.y1))
    if len(rhs._xy) > 2:
      for i in range(1, len(rhs._xy)-1):
        f1._xy.insert(-1, rhs._xy[i])
    return f1
...
Returns a new function, this function remains unchanged."""
    f1 = PieceWiseLinear(self.x0, lhs*self.y0, self.x1, lhs*self.y1)
    if len(self._xy) > 2:
      for i in range (1, len(self._xy)-1):
        f1._xy.insert(i, (self._xy[i][0], lhs*self._xy[i][1]))
    return f1
...
Returns a new function, this function remains unchanged."""
    ...
    f1 = PieceWiseLinear(x0, rhs(x0)*factor+self(x0), x1, rhs(x1)*factor+self(x1))
    if len(self._xy) > 2:
      for i in range(1, len(self._xy)-1):
        if self._xy[i][0] < x1 and self._xy[i][0] > x0:
          f1._xy.insert(i, (self._xy[i][0], factor*rhs(self._xy[i][0])+self._xy[i][1]))
    if len(rhs._xy) > 2:
       for i in range(1, len(rhs._xy)-1):
         if rhs._xy[i][0] < x1 and rhs._xy[i][0] > x0 and rhs._xy[i][0] != f1._xy[-2][0]:
           if rhs._xy[i][0] < f1._xy[-2][0]:
             f1._xy.insert(-2, (rhs._xy[i][0], factor*rhs._xy[i][1] + self(rhs._xy[i][0])))
           else:
             f1._xy.insert(-1, (rhs._xy[i][0], factor*rhs._xy[i][1] + self(rhs._xy[i][0])))
    return f1
...
This function remains unchanged."""
    f1 = PieceWiseLinear(self.x0, factor*rhs+self.y0, self.x1, factor*rhs+self.y1)
    if len(self._xy) > 2:
      for i in range (1, len(self._xy)-1):
        f1._xy.insert(i, (self._xy[i][0], factor*rhs+self._xy[i][1]))
    return f1
